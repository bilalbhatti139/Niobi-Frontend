...
class CometChatUserProfile extends React.Component {
  ...
  componentDidMount() {
    CometChat.getLoggedinUser()
      .then((user) => {
        console.log('🚀 ~ file: index.js ~ line 49 ~ CometChatUserProfile ~ .then ~ user', user);
      ...
      userProfile = (
        <React.Fragment>
          ...
          <div css={detailStyle()} className="userinfo__detail">
            <div css={thumbnailStyle()} className="detail__thumbnail hexa">
              <div className="hex1">
                <div className="hex2">{avatar}</div>
              </div>
            </div>
            <div
              css={userDetailStyle()}
