...
import { getSubscribers } from '../../../../../../api/UsersApi';
...
class CometChatUserList extends React.PureComponent {
  ...
  componentDidMount() {
    this.getSubscribers();
    this.item =
      this.getContext().type === CometChat.ACTION_TYPE.TYPE_USER ? this.getContext().item : null;
  ...
  render() {
    ...
    const users = userList.map((user) => {
      ...
      console.log(user);
      if (!this.state?.subscribers?.includes(user?.uid)) return;
      return (
    ...
    console.log('🚀 ~ file: index.js ~ line 371 ~ CometChatUserList ~ render ~ users', users);
      ...
      <div css={contactWrapperStyle(this.props, theme)} className="contacts">
        <div css={contactHeaderStyle(theme)} className="contacts__header">
          ...
          <h4
            ...
            {Translator.translate('Followers', this.props.lang)}
          </h4>
          <div></div>
        </div>
        {searchUser}
