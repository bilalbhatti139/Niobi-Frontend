...
const Q10 = (props) => {
  ...
  const [pair, setPair] = useState({ q101: '', q102: '' });
  ...
  const handleCardClick = useCallback(
    (item) => {
      ...
      if (activeQues === 0) {
        setPair(
          !isEmpty(selected) ? { ...pair, q101: selected.id.toString() } : { ...pair, q101: '' }
        );
      } else {
        setPair(
          !isEmpty(selected) ? { ...pair, q102: selected.id.toString() } : { ...pair, q102: '' }
        );
      }
  ...
  return (
    <div>
      <div className="flex flex-col  mt-48 text-xl md:text-4xl font-medium">
        <div className="flex justify-center">{split(questionsData.questions, 'to')[0]}</div>
        <div className="flex justify-center">{'to ' + split(questionsData.questions, 'to')[1]}</div>
      </div>
      ...
      <div className="flex justify-between px-8 py-8 items-center gap-2">
        ...
        <div className=" items-center rounded-3xl">
          <button
            onClick={() => {
              if (!isEmpty(pair) && !isEmpty(pair.q101) && !isEmpty(pair.q102)) {
                dispatch(updateAssessmentRequest({ type: 'question-10-11', answer: [pair] }));
