...
const RequestSessions = () => {
    ...
    (async () => {
      if (usersResponse) {
        let events = await getAllEventsByParams({
          ...
          metadata_pair: 'status:tentative'
        });
        setEvents(events);
      }
    })();
  ...
  return (
    <>
      {events
        ?.filter((_) => _.metadata?.status === 'tentative')
        .map((person) => (
          <>
            <div className="mb-3 mt-10 grid grid-cols-[0.5fr_5fr_1fr] gap-3 space-x-3 rounded-lg border border-slate-200 bg-white px-6 py-5">
              ...
              <div className="call-container my-4">
                <button
                  onClick={() => {
                    updateStatus(person, 'confirmed');
                  }}
                ...
                <button
                  onClick={() => {
                    updateStatus(person, 'cancelled');
                  }}
                  className="text-sm font-medium text-[#374151] w-100 lg:w-48   px-2  py-3 bg-gradient-to-r  rounded-md">
                  Deny
                </button>
              </div>
            </div>
