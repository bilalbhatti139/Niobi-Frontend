...
import { allsessions } from '../../../utils/constants';
import { getAllEventsByParams } from '../../../utils/NylasHelper';
...
const SessionCard = () => {
    ...
    (async () => {
      if (usersResponse) {
        let events = await getAllEventsByParams({
          ...
          metadata_pair: 'status:confirmed'
        });
        setEvents(events);
      }
    })();
  ...
  return (
    <>
      {events
        ?.filter((_) => _.metadata?.status === 'confirmed')
        ?.map((person) => (
          <>
            <div className="mb-3 mt-10 grid grid-cols-[0.5fr_5fr_1fr] gap-3 space-x-3 rounded-lg border border-slate-200 bg-white px-6 py-5">
              ...
              <div className="call-container">
                {person?.when?.start_time > moment().unix() &&
                  person?.when?.start_time <
                    moment()
                      .hour(moment().hour() + 1)
                      .unix() && (
                    <>
                      <button
                        ...
                        className="text-sm font-medium text-white w-100 lg:w-48   mb-4 px-2  py-3 bg-gradient-to-r from-pink-700 to-purple-800  rounded-md">
                        Join Call
                      </button>
                      <button className="text-sm border border-[#D1D5DB]  font-medium text-[#374151] w-100 lg:w-48   mb-4 px-2  py-3 bg-gradient-to-r  rounded-md">
                        Rescheudle
                      </button>
