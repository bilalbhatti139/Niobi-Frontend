...
export const GroupSessionModal = ({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isValidForm()) {
      let participants = guest?.map((item) => {
        return { email: item?.value, name: '' };
      });
      if (isEditMode) {
              ...
              <div className="flex items-center col-span-12">
                <div className="w-full">
                  <div className="flex items-center py-4 relative border-b-2 p-3">
                    <div className="bg-[#F28D78]/20 py-4 px-5 rounded-lg">
                      <img src={sessionImages.groupSessionIcon} alt="group session" />
                    </div>
                    <div className="ml-5">
